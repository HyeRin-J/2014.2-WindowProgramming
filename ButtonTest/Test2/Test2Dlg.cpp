...
void CTest2Dlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_BUTTON1, reset);
}
...
void CTest2Dlg::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
...
void CTest2Dlg::MoveSlider(char input)
{
	if(input == 'z'){
		pos1 += 10;
		slider1.SetPos(pos1);
	...
	if(slider1.GetPos() == 100){
		AfxMessageBox(_T("Z Win!"));
		Reset();
	}
	else if(slider2.GetPos() == 100){
		AfxMessageBox(_T("R Win!"));
		Reset();
	}
	else if(slider3.GetPos() == 100){
		AfxMessageBox(_T("M Win!"));
		Reset();
	}
	else if(slider4.GetPos() == 100){
		AfxMessageBox(_T("P Win!"));
		Reset();
	}
}


void CTest2Dlg::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
